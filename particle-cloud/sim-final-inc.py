...
def generate_uniform_distribution(e, q):
    ...
    for ei in e:
        for qi in q:
            inc = np.random.random() * np.pi/180 * 46.8
            results.append([qi, ei, qi / (1 - ei), inc])
...
print("max eccentricity: ", max(e_generate))
...
for i in range(400):
    ...
    sim.add(a=orbital_elements[i,2], e=orbital_elements[i,1], inc=orbital_elements[i,3], Omega=rand1, omega=rand2, f=rand3)
...
# sim.save_to_file("inc-test.bin", interval=1000)
...
sim = rebound.Simulationarchive("archives/inc-final.bin")
...
plt.scatter(proper_elements_initial[:,0], proper_elements_initial[:,2])
plt.show()
'''
