...
def generate_a_stripe(a_val):
    e = np.arange(0, 0.95, 0.005)
    a = np.ones_like(e) * a_val
...
def custom_integration(e, a, dt, r_hill_crit, t_sim, filenames):
    ...
    progress_bar = [
        tqdm(total=t_sim, desc=f"Thread {i + 1} Progress", position=i) for i in range(2)
    ]
    ...
    def integrate(sim, e_filter, progress, filename):
        ...
        for ei, ai in zip(e, a):
            if e_filter(ei):
                rand = np.random.random() * 2 * np.pi
                sim.add(
                    a=ai, e=ei, primary=sim.particles[0], Omega=0, omega=rand, f=rand
                )
        ...
        print("dt: ", sim.dt)
        sim.ri_ias15.min_dt = (
            1e-6  # ensure that close encounters do not stall the integration
        )
        ...
        for ts in range(0, int(t_sim), 1):  # int(sim.dt))
            ...
            if ts % 1000 == 0:  # Adjust the frequency of removals
                ...
                particles_to_remove = [
                    p for p in sim.particles[1:] if p.e > 1.02
                ]  # & (p.a*(1-p.e**2))/(1-p.e*np.cos(p.f)) > 35]
    ...
    threads = [
        Thread(target=integrate, args=(sim, f, p, fn))
        for sim, f, p, fn in zip(
            [sim1, sim2],
            [lambda e: e <= 0.4, lambda e: e > 0.4],
            progress_bar,
            filenames,
        )
    ]
...
q_generate = np.arange(
    ...
    a_scatterer + (11 * r_hill(a_scatterer, M_scatterer, M_sun)),
    0.5,
)
...
# for a-stripe
orbital_elements = generate_a_stripe(31)

print(f"added {len(orbital_elements)} test particles")

# Integrate test-population
custom_integration(
    orbital_elements[:, 0],  # e
    orbital_elements[:, 1],  # a
    1,  # dt
    2,  # r_hill_crit
    1e6,  # tsim
    [
        "archives/mercurius-a-stripe_a_31-10e6A.bin",
        "archives/mercurius-a-stripe_a_31-10e6B.bin",
    ],
)
